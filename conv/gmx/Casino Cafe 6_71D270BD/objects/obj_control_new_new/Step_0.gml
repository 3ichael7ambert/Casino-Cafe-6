...
switch(int_state){
    ...
    switch(int_subState){
        ...
        case 0:
        if (alarm[0] == -1){
            if (int_chips >= 10){//Keep betting if I have at least 10 chips       
                int_round ++;
                int_chips -= 10;//Take poker chips
                int_subState ++;//Move onto next substate
            }else{
                instance_create(room_width / 2, room_height / 2 + 15, obj_youLose);//Create the exclaimation 'You Lose!' in the center
